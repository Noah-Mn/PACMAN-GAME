...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #   x,y = currentPosition
            #   dx, dy = Actions.directionToVector(action)
            #   next_x, next_y = int(x + dir _x), int(y + dir_y)
            ...
            new_cornerVisited = cornerVisited[:]
            isWall = self.walls[new_X, new_Y]
            if not isWall:
                cornerIndex = 0

                for corner in self.corners:
                    if (new_X, new_Y) == corner:
                        break
                        cornerIndex += 1
                if cornerIndex < 4:
                    new_cornerVisited[cornerIndex] = True
                    new_state = ((new_X, new_Y), new_cornerVisited)
                    cost = self.costFn(new_X, new_Y)
...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):
        ...
        closestIndex = 0
        cornerIndex = 0
        for cornerDistance in cornersDistance:
            if visitedList[closestIndex]:
                closestIndex = cornerIndex

            if (not visitedList[cornerIndex]) and (cornerDistance < cornersDistance[cornerIndex]):
                closestIndex = cornerIndex
            cornerIndex += 1

        if not visitedList[closestIndex]:
            V += cornersDistance[cornerIndex]
